...
def idt_device(lib: gd.GdsLibrary, pads: gd.Cell, layers: dict, lmda: float,
        ...
        if g_idt == 0:  # one-port IDT
            ...
            if idt_type == IDT_Type.OPEN:
                ...
                c.add(gd.CellReference(idts, origin=(-idt_offset, 0),
                                       rotation=180))  # rotation puts idt finger at gap edge
                c.add(gd.CellReference(idts, origin=(idt_offset, 0), rotation=0))
...
def IDT_test(lib, layers, label_layer, pitches, mratios):
    ...
    for p in pitches:
        for mr in mratios:
            ...
            current_x = current_x+d.get_bounding_box()[1][0]-d.get_bounding_box()[0][
                0]+p
...
g_idts = [5, 10, 20, 50, 100]

# for mr in [0.5]:  # [0.3, 0.4, 0.5, 0.75]:
#     for l in lmbdas:
#         for n in [10]:
#             idt_device(lib, pads2, layers, lmda=l, g_idt=10,
#                        idt_type=IDT_Type.OPEN,
#                        l_idt=50*l,
#                        n_idt=n,
#                        s_b=l,
...
for mr in [0.3, 0.4, 0.5, 0.75]:
        for l in lmbdas:
            for n in [10, 20, 40]:
                ...
                #           cell_prefix='')
                for g in g_idts:
                    for d in [IDT_Type.STANDARD, IDT_Type.OPEN]:
